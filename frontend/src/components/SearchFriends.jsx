...
export default function SearchFriends() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(`https://social-hub-sqid.onrender.com/users/search-friends?query=${search}&userId=${user?.userId}`)
      const data = await res.json()
  ...
  const handleAddFriend = async (friendId) => {
    try {
      const res = await fetch('https://social-hub-sqid.onrender.com/users/add-friend', {
        method: 'POST',
  ...
  const handleRemoveFriend = async (friendId) => {
    try {
      const res = await fetch('https://social-hub-sqid.onrender.com/users/remove-friend', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: user?.userId, friendId })
      })
